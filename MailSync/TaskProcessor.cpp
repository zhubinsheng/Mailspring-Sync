...
void TaskProcessor::performLocal(Task * task) {
    ...
    try {
        ...
        else if (cname == "SyncbackSummaryTask") {
            if (!task->data().count("threadId")) return;
            store->handleSummaryUpdate(task->data(), account);
        } else if (cname == "DestroySummaryTask") {
            if (!task->data().count("threadId")) return;
            store->handleSummaryDelete(task->data(), account);
        } else if (cname == "SyncbackContactRelationTask") {
            if (!task->data().count("email")) return;
            store->handleContactRelationUpdate(task->data(), account);
        ...
        try {
            if (cname == "InsertAiChatSessionTask") {
                aiChatStore->insert(task->data());
            } else if (cname == "UpdateAiChatSessionTask") {
                aiChatStore->update(task->data());
            } else if (cname == "DestroyAiChatSessionTask") {
                aiChatStore->remove(task->data());
            } else if (cname == "InsertAiMessageTask") {
                aiChatStore->insertAiMessage(task->data());
            } else if (cname == "UpdateAiMessageTask") {
                aiChatStore->updateAiMessage(task->data());
            } else if (cname == "DeleteAiMessageTask") {
                aiChatStore->deleteAiMessage(task->data());
            } else {
